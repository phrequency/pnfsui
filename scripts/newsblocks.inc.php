 ...
 */
define('NB_FAVICON_DEFAULT', './img/alternate_favicon.png');
define('NB_NEW_HTML', '<span class="new"></span>');
...
class newsblocks
{
	...
	function listing($url, $options = null)
	{
		...
		$patterns[0] = '/\s+/';
		$patterns[1] = '/[.]/';
		$id = 'x' . preg_replace($patterns, '', $title);
		...
		for ($x = 0; $x < 2; $x++)
		{
			...
			$html .= '<ul data-parent="'. $id .'" id="' . ($secondary ? 's_' : 'p_') . $id . '" class="' . ($secondary ? 'secondary' : 'primary') . '" style="direction:' . $direction . ';' . ($secondary ? 'display:none;' : '') . '">' . "\n";
...
foreach ($feed->get_items($counter_start, $counter_length) as $item)
		{
			...
			if ($enclosure = $item->get_enclosure())
			{
				...
				$html .= '<li class="citem"><div class="item-inner">';
				$html .= '<a href="' . $item->get_permalink() . '" target="_blank">'; // <a> tag link to story.
				$html .= $item->get_title(); // The title for the link
				$html .= '</a>'; // Close the </a>
				$html .= '<div class="descr"><div class="descr-inner">';
				if ($thumb) {
				$html .= '<img src="' . $thumb . '" border="0" class="item-thumb" />';
				}
				$html .= $title_attr.'</div></div>'; // <div> tag with the short description.
				$html .= $new . '</div></li>' . "\n"; // Close the </li>
			...
			else
			{
				...
				$html .= $item->get_title(); // The title for the link
				$html .= '</a>'; // Close the </a>
				$html .= '<div class="descr"><div class="descr-inner">'.$title_attr.'</div></div>'; // <div> tag with the short description.
	 ...
	 * @return string The HTML markup to display on the page.
	 */
	function listingorig($url, $options = null)
	{
		// Retrieve the data and break out the individual variables (i.e. $title and $permalink are usable)
		if (!$options) $options = array();
		extract(newsblocks::data($url, $options));
...
function quickie($url, $options = null)
	{
		// Retrieve the data and break out the individual variables (i.e. $title and $permalink are usable)
		if (!$options) $options = array();
		extract(newsblocks::data($url, $options));
		...
		for ($x = 0; $x < 2; $x++)
		{
			...
			$html .= ($secondary ? '<div class="q_secondary"><img src="./img/q_arrow.gif" class="q_arrow"><span class="close-wrap"><a href="#" class="close">[x]</a></span><span class="source"><a href="' . $permalink . '" target="_blank">more from '. $title .'</a></span>' : '<span class="q_primary">') . '<ul>';
			...
			foreach ($feed->get_items($counter_start, $counter_length) as $item)
			{
				...
				$short_title = substr($feed_title, 0,38); 

				// Add each item: item title, linked back to the original posting, with a tooltip containing the description.
				$html .= '<li class="citem"><div class="item-inner">';
				$html .= '<a href="' . $item->get_permalink() . '" target="_blank">'; // <a> tag link to story.
				$html .=  $short_title; // The title for the link
				$html .= '</a>'; // Close the </a>
				$html .= '<div class="descr"><div class="descr-inner"><strong>'.$item->get_title().'</strong><br />'.$title_attr.'</div></div>'; // <div> tag with the short description.
				$html .= '</li>';
			}

			// Close out of the primary list
			$html .= '</ul>' . "\n";
			$html .= ($secondary ? '</div>' : '</span>');		
		}

		// Close out of this <div> q_block.
		$html .= '</div>' . "\n";
		...
		foreach ($feed->get_items(0, $items) as $item)
		{
			...
			if ($enclosure = $item->get_enclosure())
			{
				...
				if ($thumb = $enclosure->get_thumbnail())
				{
					...
					//$html .= '<a href="' . $item->get_permalink() . '" title="' . $title_attr . '" target="_blank">'; // <a> tag with a short description in the title attribute.
...
*/
	...
	function comic($url, $options = null)
	{
		// Retrieve the data and break out the individual variables (i.e. $title and $permalink are usable)
		if (!$options) $options = array();
		extract(newsblocks::data($url, $options));
